    ...
    HSDataTable = class _HSDataTable extends HSBasePlugin {
      ...
      destroy() {
        ...
        if (this.pageEntitiesList)
          this.onPageEntitiesChangeListener.forEach(({ el, fn: fn2 }) => el.removeEventListener("change", fn2));
        if (this.pagingPagesList.length) {
        ...
        if (this.pagingPrevList.length)
          this.onPagingPrevClickListener.forEach(({ el, fn: fn2 }) => el.removeEventListener("click", fn2));
        if (this.pagingNextList.length)
          this.onPagingNextClickListener.forEach(({ el, fn: fn2 }) => el.removeEventListener("click", fn2));
...
var _HSAccordion = class _HSAccordion extends HSBasePlugin {
  ...
  // Public methods
  toggleClick(evt) {
    if (this.isToggleStopPropagated)
      evt.stopPropagation();
  ...
  static on(evt, target, cb) {
    const instance = _HSAccordion.findInCollection(target);
    if (instance)
      instance.element.events[evt] = cb;
  }
};
...
var HSCollapse = class _HSCollapse extends HSBasePlugin {
  ...
  static findInCollection(target) {
    return window.$hsCollapseCollection.find((el) => {
      if (target instanceof _HSCollapse)
        return el.element.el === target.el;
      else if (typeof target === "string")
        return el.element.el === document.querySelector(target);
      else
...
var HSComboBox = class _HSComboBox extends HSBasePlugin {
  ...
  setValueAndClear(val, data = null) {
    if (val)
      ...
      this.setValue(this.selected, data);
  ...
  setResultAndRender(value = "") {
    ...
    if (this.apiSearchQuery || this.apiSearchPath || this.apiSearchDefaultPath)
      this.itemsFromJson();
    if (_value === "")
      this.isSearchLengthExceeded = true;
    else
      this.isSearchLengthExceeded = false;
  }
  setResults(val) {
    this.value = val;
    this.resultItems();
...
var _HSDropdown = class _HSDropdown extends HSBasePlugin {
  ...
  static findInCollection(target) {
    return window.$hsDropdownCollection.find((el) => {
      if (target instanceof _HSDropdown)
        return el.element.el === target.el;
      else if (typeof target === "string")
        return el.element.el === document.querySelector(target);
      else
  ...
  static onArrowX(evt, direction) {
    ...
    if (isRootDropdown && !futurePosition.includes(direction))
      return false;
...
var _HSLayoutSplitter = class _HSLayoutSplitter extends HSBasePlugin {
  ...
  buildControl(prev, next, direction = "horizontal") {
    ...
    if (direction === "horizontal")
      this.horizontalControls.push(item);
    else
      this.verticalControls.push(item);
    ...
    this.horizontalSplitters.forEach(({ items }) => {
      ...
      items.forEach((el, index) => {
        if (index >= items.length - 1)
          this.buildControl(el, null);
        else
          this.buildControl(el, items[index + 1]);
      });
    });
  }
  ...
  updateVerticalSplitter() {
    this.verticalSplitters.forEach(({ items }) => {
      ...
      items.forEach((el, index) => {
        if (index >= items.length - 1)
          this.buildControl(el, null, "vertical");
        else
          this.buildControl(el, items[index + 1], "vertical");
      });
    });
  }
  ...
  static findInCollection(target) {
    return window.$hsLayoutSplitterCollection.find((el) => {
      if (target instanceof _HSLayoutSplitter)
        return el.element.el === target.el;
      else if (typeof target === "string")
        return el.element.el === document.querySelector(target);
      else
...
__publicField(_HSLayoutSplitter, "onDocumentPointerMove", (evt) => {
  ...
  if (!draggingInstance || !draggingInstance.element.isDragging)
    return;
...
__publicField(_HSLayoutSplitter, "onDocumentPointerUp", () => {
  ...
  if (draggingInstance)
    draggingInstance.element.controlPointerUp();
});
...
var HSOverlay = class _HSOverlay extends HSBasePlugin {
  ...
  static findInCollection(target) {
    return window.$hsOverlayCollection.find((el) => {
      if (target instanceof _HSOverlay)
        return el.element.el === target.el;
      else if (typeof target === "string")
        return el.element.el === document.querySelector(target);
      else
...
var HSSelect = class _HSSelect extends HSBasePlugin {
  ...
  async remoteSearch(val) {
    if (val.length <= this.minSearchLength) {
      ...
      if (res2.length)
        this.buildOptionsFromRemoteData(res2);
      else
  ...
  recalculateDirection() {
    var _a, _b, _c, _d;
    if ((this == null ? void 0 : this.dropdownVerticalFixedPlacement) && (this.dropdown.classList.contains("bottom-full") || this.dropdown.classList.contains("top-full")))
      return false;
  ...
  static findInCollection(target) {
    return window.$hsSelectCollection.find((el) => {
      if (target instanceof _HSSelect)
        return el.element.el === target.el;
      else if (typeof target === "string")
        return el.element.el === document.querySelector(target);
      else
  ...
  static open(target) {
    ...
    if (instance && !instance.element.isOpened)
      instance.element.open();
  }
  static close(target) {
    const instance = _HSSelect.findInCollection(target);
...
var HSTextareaAutoHeight = class _HSTextareaAutoHeight extends HSBasePlugin {
  ...
  parentType() {
    if (this.el.closest(".hs-collapse"))
      ...
      return "overlay";
    else if (this.el.closest('[role="tabpanel"]'))
      return "tabs";
    else
      return false;
  }
    ...
    if (this.parentType() === "tabs") {
      ...
      element.on("change", (payload) => {
        ...
        textareas.forEach((el) => {
          ...
          if (instance)
            instance.element.textareaSetHeight(3);
        });
      });
...
var HSTooltip = class _HSTooltip extends HSBasePlugin {
  ...
  static findInCollection(target) {
    return window.$hsTooltipCollection.find((el) => {
      if (target instanceof _HSTooltip)
        return el.element.el === target.el;
      else if (typeof target === "string")
        return el.element.el === document.querySelector(target);
      else
...
var _HSTreeView = class _HSTreeView extends HSBasePlugin {
  ...
  static findInCollection(target) {
    return window.$hsTreeViewCollection.find((el) => {
      if (target instanceof _HSTreeView)
        return el.element.el === target.el;
      else if (typeof target === "string")
        return el.element.el === document.querySelector(target);
      else
